...
function socplan_opt(type)
  ...
  system(['cp temp_files' filesep 'policy.txt temp_files' filesep 'socplan' type '-' alg.ptag '.txt']);
  ...
  function score = socplan_obj(parin)
    ...
    if (any(params < opt.plb) | any(params > opt.pub))
      ...
      if (score < opt.bestval)
        opt.bestval = score;
        opt.lastseq = seqfin;
        system(['cp temp_files' filesep 'policy.txt temp_files' filesep 'policy_bestSoc.txt']);
