function [score,moments] = compMoments()
  ...
  if alg.bootstrap == 0
    ...
    dat_full = csvread([alg.mmtBoot_file]);    % bootstrapped moments are used
  end  
  ...
  if alg.bootstrap == 0
    ...
    % -----------------------------------------------------
    % Reporting nontargeted moments
    % -----------------------------------------------------
    fileName = ['temp_files' filesep 'non_targetedPanelB-' alg.ptag '.txt'];
    fid = fopen(fileName,'w');
    fprintf(fid,'Panel B\n');
    fprintf(fid,'%-40s %1.3f\n','R&D per employee ratio (high to low)',ntm.rdExpPerEmpEstabHigh/ntm.rdExpPerEmpEstabLow);
    fprintf(fid,'%-40s %1.3f\n','Patent per employee (high to low)'   ,ntm.patPerEmpEstabHigh/ntm.patPerEmpEstabLow);
    fprintf(fid,'\n');
    fclose(fid);

    fileName = ['temp_files' filesep 'non_targetedPanelC-' alg.ptag '.txt'];
    fid = fopen(fileName,'w');
    fprintf(fid,'Panel C\n');
    fprintf(fid,'%-40s %1.3f\n', 'Productivity dist. - 75/25: small/young',ntm.tfpSM.quan11(2)/ntm.tfpSM.quan11(1));
    fprintf(fid,'%-40s %1.3f\n', 'Productivity dist. - 75/25: small/old'  ,ntm.tfpSM.quan12(2)/ntm.tfpSM.quan12(1));
    fprintf(fid,'%-40s %1.3f\n', 'Productivity dist. - 75/25: large/young',ntm.tfpSM.quan21(2)/ntm.tfpSM.quan21(1));
    fprintf(fid,'%-40s %1.3f\n', 'Productivity dist. - 75/25: large/old'  ,ntm.tfpSM.quan22(2)/ntm.tfpSM.quan22(1));
    fprintf(fid,'\n');
    fclose(fid);

    fileName = ['temp_files' filesep 'non_targetedPanelD-' alg.ptag '.txt'];
    fid = fopen(fileName,'w');
    fprintf(fid,'Panel D\n');
    fprintf(fid,'%-40s %1.3f\n', 'Add a product (or more)'  ,ntm.product.soleAdd);
    fprintf(fid,'%-40s %1.3f\n', 'Drop a product (or more)' ,ntm.product.soleDrop);
    fprintf(fid,'%-40s %1.3f\n', 'Do both'                  ,ntm.product.either);
    fprintf(fid,'%-40s %1.3f\n', 'Do neither'               ,ntm.product.neither);
    fprintf(fid,'--------------------------------------------------------\n');
    fprintf(fid,'\n\n');
    fclose(fid);

    fileName = ['temp_files' filesep 'prodLineDist-' alg.ptag '.txt'];
    fid = fopen(fileName,'w');
    fprintf(fid,'PRODUCT LINE DIST.\n');
    fprintf(fid,'-------------------------------\n');
    fprintf(fid,'%10s %10s\n','# of products', 'share');
    for i = 1:9
      fprintf(fid,'%10i %12.2f\n',i,ntm.prodDist(i));
    end
    fprintf(fid,'%10s %12.2f\n','10+',sum(ntm.prodDist(10:end)));
    fprintf(fid,'--------------------------------------------------------\n');
    fprintf(fid,'\n\n');
    fclose(fid);
  end 
