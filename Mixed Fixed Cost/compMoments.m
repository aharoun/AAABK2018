function [score,moments] = compMoments()
  ...
  dfwage_manag  = (eq.lS * eq.ws + eq.lU*eq.wu)*dfnprod;
  dfwage_prod   = wage_bill_cf*dfqeps1.*eq.Lp;      % adjusted for unskill production workers;
  dfwage_rnd    = eq.ws*eq.cx(dftype+1).*dfnprod;
  dfwage_tot    = dfwage_manag + dfwage_prod + dfwage_rnd;

  % worker counts
  labor_cf      = ((p.eps-1.0)/p.eps)^(p.eps-1.0);
  dflabor_manag = (eq.lU + eq.lS)*dfnprod;
  dflabor_prod  = labor_cf*dfqeps1.*eq.Lp;      % adjusted for unskill production workers;
  ...
  fixedtoRDLab = eq.cactivtot*(eq.lS + eq.lU)/eq.crnd;
