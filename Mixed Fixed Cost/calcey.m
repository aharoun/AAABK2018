...
function calcey
  ...
  eq.eyqAll   = alg.w'*eyfunc(alg.chebBin); % taking expectation
...
function eyout = eyfunc(q)
  ...
  eyout = [zfunc(qq,0.0,1).*out' (zfunc(qq,p.nu,2) + zfunc(qq,0.0,1) - zfunc(qq,p.nu,1)).*out'];
          % LOW TYPE                 % HIGH TYPE  
end
...
function zout = zfunc(q,x,s)
  ...
  coeff1 = eq.Lp*p.picf;
  coeff2 = eq.optval(s) - eq.fixedCost;
